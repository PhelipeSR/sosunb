...
class Lista extends CI_Controller {
	...
	public function render() {
		$this->load->view('admin/menu/lista');
	}
	...
	public function create_status() {
		...
		$this->form_validation->set_rules('name', 'Lista', 'required|max_length[50]');
		...
		if ( $this->form_validation->run() ) {
			...
			if ($this->Lista_model->create_status($database) ) {
	...
	public function update_status() {
		...
		if ( $this->form_validation->run() ) {
			...
			if ($this->Lista_model->update_status($database, $this->input->post('id')) ) {
				$this->saida->set_dados('ok');
			}else {
	...
	public function delete_status() {
		...
		if ( $this->form_validation->run() ) {
			$this->load->model('admin/menu/Lista_model');
			if ($this->Lista_model->delete_status($this->input->post('excluded'), $this->input->post('id'))) {
				$this->saida->set_dados('ok');
			}else {
