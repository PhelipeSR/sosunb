...
$(document).ready(function() {
	...
	var campus_table = $('#campus_table').DataTable({
		dom: "<'row'<'col-sm-6'l><'col-sm-6'f>>B"+"<'row'<'col-sm-12'tr>>"+"<'row'<'col-sm-5'i><'col-sm-7'p>>",
		ajax: { url: base_url('admin/menu/campus/get_campus'), type: "post"},
	...
	$(document).off('click', '.excluir').on('click', '.excluir', function(){
		...
		$.ajax({
			url: base_url('admin/menu/campus/delete_campus'),
			method: 'post',
			dataType: 'json',
			data: {
				id: id,
			...
			success: function(data, textStatus, jqXHR) {
				if (data.erro) {
					...
					campus_table.ajax.reload( null, false );
				}
			},
			beforeSend: function(){
	...
	$(document).off('click', '.editar').on('click', '.editar', function(){
		resset_form('#formEditCampus');
		var row = campus_table.row( $('[data-row="'+$(this).data('btn')+'"]') ).data();
		$('#id').val(row.id);
		$("#campus_edit").val(row.campus);
	...
	*  ===================================================================== */
	...
	$( "#formAddCampus" ).validate( {
		errorClass: 'invalid-feedback',
		rules: {
			campus:   {required: true,maxlength: 50},
		},
		...
		submitHandler: function (form) {
			$.ajax({
				url: base_url('admin/menu/campus/create_campus'),
				method: 'post',
				dataType: 'json',
	...
	$( "#formEditCampus" ).validate( {
		...
		submitHandler: function (form) {
			$.ajax({
				url: base_url('admin/menu/campus/update_campus'),
				method: 'post',
				dataType: 'json',
				data: $('#formEditCampus').serialize(),
				success: function(data, textStatus, jqXHR) {
					if (data.erro) {
						toastr.error(data.msg_erro, "Falha");
					}else if(data.dados == 'ok') {
