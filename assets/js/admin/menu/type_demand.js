...
$(document).ready(function() {
	...
	var type_demand_table = $('#type_demand_table').DataTable({
		dom: "<'row'<'col-sm-6'l><'col-sm-6'f>>B"+"<'row'<'col-sm-12'tr>>"+"<'row'<'col-sm-5'i><'col-sm-7'p>>",
		ajax: { url: base_url('admin/menu/type_demand/get_type_demand'), type: "post"},
	...
	$(document).off('click', '.excluir').on('click', '.excluir', function(){
		...
		$.ajax({
			url: base_url('admin/menu/type_demand/delete_type_demand'),
			method: 'post',
			dataType: 'json',
			data: {
				id: id,
			...
			success: function(data, textStatus, jqXHR) {
				if (data.erro) {
					...
					type_demand_table.ajax.reload( null, false );
				}
			},
			beforeSend: function(){
	...
	$(document).off('click', '.editar').on('click', '.editar', function(){
		resset_form('#formEditTypeDemand');
		var row = type_demand_table.row( $('[data-row="'+$(this).data('btn')+'"]') ).data();
		$('#id').val(row.id);
		$("#demands_edit").val(row.demands);
	...
	*  ===================================================================== */
	...
	$( "#formAddTypeDemand" ).validate( {
		errorClass: 'invalid-feedback',
		rules: {
			demands:   {required: true,maxlength: 50},
		},
		...
		submitHandler: function (form) {
			$.ajax({
				url: base_url('admin/menu/type_demand/create_type_demand'),
				method: 'post',
				dataType: 'json',
				data: $('#formAddTypeDemand').serialize(),
				success: function(data, textStatus, jqXHR) {
					if (data.erro) {
						toastr.error(data.msg_erro, "Falha");
					}else if(data.dados == 'ok') {
	...
	$( "#formEditTypeDemand" ).validate( {
		...
		submitHandler: function (form) {
			$.ajax({
				url: base_url('admin/menu/type_demand/update_type_demand'),
				method: 'post',
				dataType: 'json',
				data: $('#formEditTypeDemand').serialize(),
				success: function(data, textStatus, jqXHR) {
					if (data.erro) {
						toastr.error(data.msg_erro, "Falha");
					}else if(data.dados == 'ok') {
